...
// then rotates 49 deg CW (right) to face the hub, then shoots.
...
public class ShootInPlaceLeftBotRotateAuto extends SequentialCommandGroup {
  ...
  public ShootInPlaceLeftBotRotateAuto(
    ...
    setName("ShootInPlaceLeftBotRotate");
    ...
    addCommands(
        // Seed pose — same start position as ShootInPlaceLeft, flipped for Red
        Commands.runOnce(() -> {
          Pose2d startPose = poseEstimator.getStartPoseForAutoName("ShootInPlaceLeftBotRotate");
        ...
        // Rotate bot 49 deg CW (right) from current gyro heading to face hub
        Commands.runOnce(() -> {
          double currentRad = drive.getGyroRotation().getRadians();
          double goalRad    = currentRad - Math.toRadians(ROTATE_RIGHT_DEG);
